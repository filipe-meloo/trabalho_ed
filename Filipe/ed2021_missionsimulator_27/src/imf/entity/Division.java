...
public class Division {
    ...
    public Double getEnemiesPower() {
        ...
        for (int j = 0; j < this.numEnemies; j++) {
            power += this.getEnemies()[j].getPower();
        }
        return power;
    }
    ...
    public boolean isValid() {
        if (this.numEnemies > 0) {
            return (this.name != null && !this.name.isBlank() && this.numEnemies != null && this.numEnemies >= 0 && Enemy.isValid(this.enemies));
        }
        return (this.name != null && !this.name.isBlank() && this.numEnemies != null && this.numEnemies >= 0);
    }

    /**
     * Verifica se um array de divisões possui um formato valido. Para o array
     * de divisões ser valido cada instancia da divisão deve ser diferente de
     * nula e simultanemente valida.
     *
     * @param div array de divisões que se pretende validar.
     * @return verdadeiro se a divisao for valida, falso caso contrario.
     */
    public static boolean isValid(Division[] div) {
        boolean send = true;
