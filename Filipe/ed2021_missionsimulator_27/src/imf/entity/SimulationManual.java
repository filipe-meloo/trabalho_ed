...
public class SimulationManual implements Comparable {
     ...
     * resgatado.
     */
    public SimulationManual(Double lifePoints, LinkedQueue<Division> path, boolean target) {
        this.lifePoints = lifePoints;
        this.getTarget = target;
        this.path = new String[path.size()];
        int i = 0;
        while (!path.isEmpty()) {
            Division div = (Division) path.dequeue();
    ...
    /**
     * Construtor que cria uma simulação manual.Recebe como parametro o numero
     * de vida com que a simulação foi terminada.Recebe um array de strings que
     * representa o caminho percorrido.Por ultimo recebe um valor booleano que
     * representa se o alvo foi ou não resgatado.
     *
     * @param lifePoints o numero de vida com que a simulação foi terminada.
     * @param path caminho percorrido na simulação.
     ...
     * resgatado.
     */
    public SimulationManual(Double lifePoints, String[] path, boolean target) {
        this.lifePoints = lifePoints;
        this.getTarget = target;
     ...
     */
    public void setPath(LinkedQueue<Division> path) {
        this.path = new String[path.size()];
        int i = 0;
        while (!path.isEmpty()) {
            Division div = (Division) path.dequeue();
